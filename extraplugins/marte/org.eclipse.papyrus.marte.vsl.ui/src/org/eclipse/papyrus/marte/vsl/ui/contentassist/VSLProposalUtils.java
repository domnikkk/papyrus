...
import org.eclipse.papyrus.marte.vsl.scoping.visitors.ScopingVisitors.Visitor_GetRecursivelyOwnedAndImportedMetaclassInstances;
import org.eclipse.papyrus.marte.vsl.scoping.visitors.ScopingVisitors.Visitor_GetRecursivelyOwnedAndImportedStereotypeInstances;
...
public class VSLProposalUtils {
	...
	protected static Map<String, Element> buildProposalForStereotype(Classifier classifier) {
		...
		allElements.addAll(Visitor_GetRecursivelyOwnedAndImportedStereotypeInstances.visit(VSLJavaValidator.getModel(), (Stereotype) classifier));
	...
	protected static Map<String, Element> buildProposalForMetaclass(Classifier classifier) {
		...
		allElements.addAll(Visitor_GetRecursivelyOwnedAndImportedMetaclassInstances.visit(VSLJavaValidator.getModel(), (org.eclipse.uml2.uml.Class) classifier));
		for (Element c : allElements) {
			if (c instanceof NamedElement) {
				allProposals.put(getNameLabel((NamedElement) c), c);
			}
	...
	protected static Map<String, Element> buildProposalForMetaclass(EClass metaclass) {
		...
		Map<String, Element> allProposals = new HashMap<String, Element>();
		new ScopingVisitors.Visitor_GetRecursivelyOwnedAndImportedMetaclassInstances();
		allElements.addAll(Visitor_GetRecursivelyOwnedAndImportedMetaclassInstances.visit(VSLJavaValidator.getModel(), metaclass));
		for (Element c : allElements) {
			if (c instanceof NamedElement) {
				allProposals.put(getNameLabel((NamedElement) c), c);
			}

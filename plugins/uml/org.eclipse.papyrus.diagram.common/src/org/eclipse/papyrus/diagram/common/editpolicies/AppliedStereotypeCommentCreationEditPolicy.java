...
import org.eclipse.papyrus.diagram.common.commands.CreateAppliedStereotypeCommentViewCommand;
...
public class AppliedStereotypeCommentCreationEditPolicy extends AppliedStereotypeNodeLabelDisplayEditPolicy {
	...
	protected void updateAppliedStereotypeCommentShape(){
		...
		if(getAppliedStereotypeCommentNode()==null&&(!stereotypePropertiesToDisplay.trim().equals(""))){
			...
			if (stereotypeLocalizationToDisplay.equals(VisualInformationPapyrusConstant.STEREOTYPE_COMMENT_LOCATION)){
		...
		if(getAppliedStereotypeCommentNode()!=null){
			...
			if (!stereotypeLocalizationToDisplay.equals(VisualInformationPapyrusConstant.STEREOTYPE_COMMENT_LOCATION)||getvisisbleAppliedStereotypeCompartment(commentNode,  getUMLElement())==0){
	...
	protected void delegateEAnnotationInCommentShape(){
		...
		if(appliedStereotypeComment!=null){
			...
			final EAnnotation stereotypeAnnotation=originView.getEAnnotation(VisualInformationPapyrusConstant.STEREOTYPE_ANNOTATION);
			...
			final RecordingCommand cmd=new RecordingCommand(domain,"EannotationDelegation") {
				...
				protected void doExecute() {
					...
					RecordingCommand cmd=AppliedStereotypeHelper.getSetAppliedStereotypePropertiesLocalizationCommand(domain, appliedStereotypeComment, VisualInformationPapyrusConstant.STEREOTYPE_COMPARTMENT_LOCATION);
					cmd.execute();

...
class DropTestTemplate {
	def static generate(Class clazz) {
		val template = '''
			...
			import org.eclipse.papyrus.uml.diagram.tests.drop.«clazz.generalizations.head.general.name»;
			...
			public class «clazz.name» extends «clazz.generalizations.head.
				...
				«FOR Operation testCaseOperation : clazz.ownedOperations.filter[getAppliedStereotype("utp::TestCase") != null]»
					...
					/**
					 * Test to drop element «TemplateQueries.getComponentName(instanceSpec)».
					 */
					@Test
					@GeneratedTest
					public void «testCaseOperation.name»() {
						«DropTestTemplate.testDropNode(testCaseOperation.methods.head as Activity, clazz)»
					}
				«ENDFOR»
	...
	static def String testDropNode(Activity method, Class clazz) {
		...
		val template = '''
			«action.operation.name»(UMLElementTypes.getElementType(«editPartClassName».VISUAL_ID), «TemplateQueries.UMLPackageElementType(node)», true);
		'''
		return template
	}
}

...
class EditionTestTemplate {
	def static generate(Class clazz) {
		val template = '''
			...
			import org.eclipse.papyrus.uml.diagram.tests.edition.«clazz.generalizations.head.general.name»;
			...
			public class «clazz.name» extends «clazz.generalizations.head.
				...
				«FOR Operation testCaseOperation : clazz.ownedOperations.filter[getAppliedStereotype("utp::TestCase") != null]»
					...
					/**
					 * Test the editable properties of element «TemplateQueries.getComponentName(instanceSpec)».
					 */
					@Test
					@GeneratedTest
					public void «testCaseOperation.name»() {
						«testEditionNode(testCaseOperation.methods.head as Activity, clazz)»
					}
				«ENDFOR»
			}
		'''
	...
	static def String testEditionNode(Activity method, Class clazz) {

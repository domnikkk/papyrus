...
class TestLinkTemplate {
		...
		val template = '''
			...
			public class «clazz.name» extends «clazz.generalizations.head.general.name» {
				...
				«FOR Operation testCaseOperation : clazz.ownedOperations.filter[getAppliedStereotype("utp::TestCase") != null]»
					...
					/**
					 * Test to create link «TemplateQueries.getComponentName(instanceSpec)».
					 */
					@Test
					@GeneratedTest
	...
	static def String testToManageLink(Activity method, Class clazz) {
		...
		for (ActivityNode activityNode : method.ownedNodes) {
			...
			template += '''UMLElementTypes.getElementType(«sourceType».VISUAL_ID), UMLElementTypes.getElementType(«targetType».VISUAL_ID), UMLElementTypes.getElementType(«linkType».VISUAL_ID), '''
			
			val containerArgument = action.arguments.findFirst[name == 'container']
			if(containerArgument != null){
				var container = ((containerArgument as ValuePin).value as InstanceValue).instance
				val containerType = Queries.getStringSlotValue(container, 'editPartClassName')
				template += '''UMLElementTypes.getElementType(«containerType».VISUAL_ID), '''  
			}
			template +=	'''true);

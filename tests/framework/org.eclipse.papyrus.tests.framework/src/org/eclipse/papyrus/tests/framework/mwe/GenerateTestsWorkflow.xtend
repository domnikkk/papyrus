...
class GenerateTestsWorkflow extends XtendWorkflow {
	...
	public override run(IWorkflowContext context) {
		...
		transformGmfgenToUmlPhase()

		//2. Generate Test specification model from QVTO script
		generateTestModelPhase()

		//3. Generate Test code
		generateTestCodePhase()
		super.run(context)
	}
	
	def transformGmfgenToUmlPhase() {
	...
	def generateTestModelPhase() {
		...
		components += new FileCopy => [
			sourceFile = gitRoot +
				'tests/framework/org.eclipse.papyrus.tests.framework/qvto/CreateFromPaletteTestChildLabelNodes.qvto'
			targetFile = projectUri + 'qvto/CreateFromPaletteTestChildLabelNodes.qvto'
		]
		components += new FileCopy => [
		...
		components += new FileCopy => [
			sourceFile = gitRoot + 'tests/framework/org.eclipse.papyrus.tests.framework/qvto/DeleteTests.qvto'
			targetFile = projectUri + 'qvto/DeleteTests.qvto'
		]
		components += new FileCopy => [
			sourceFile = gitRoot + 'tests/framework/org.eclipse.papyrus.tests.framework/qvto/EditionTests.qvto'
			targetFile = projectUri + 'qvto/EditionTests.qvto'
		]
		components += new FileCopy => [
			sourceFile = gitRoot +
		...
		components += new DeleteFile => [
			...
			addUri('qvto/DeleteTests.qvto')
			addUri('qvto/EditionTests.qvto')
			addUri('qvto/TestModelCreationUtilities.qvto')
		]
	}

	protected def generateTestCodePhase() {
		components += new Reader => [
			uri = testModelUri
			modelSlot = 'papyrusTestModel'
		]
